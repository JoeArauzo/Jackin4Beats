...
def trim_audiosilence(file, verbosity, namefromtag, test, end_offset, begin_offset,
    ...
    try:
        source = mutagen.File(str(audiofile))
    except mutagen.MutagenError as e:
        logger.error("Error inspecting FILE.")
        logger.error(e)
        sys.exit(4)
    if source != None:
        audio_format = type(source).__name__
        if audio_format in supported_audio_formats:
            logger.debug("FILE inspected successfully.  " +
                         f"Format: {type(source).__name__}")
        else:
            logger.error("Unsupported audio format.  Supported formats: " +
    ...
    logger.info(f"File type                  :  {audio_format}")
    ...
    if len(trimmed_sound) != duration_ms:
        ...
        if not test:
            ...
            # Restore cleaned metadata to trimmed audio file
            if len(source.tags):
                # Clear value for ENODEDBY
                if "TENC" in source.tags:
                    logger.debug("Clearing value of ENCODEDBY.")
